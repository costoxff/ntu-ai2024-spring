...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # for houses in gameState.getPossibleHouses():
    #     for obs in gameState.getHouseWalls(houses):
    ...
    pacman_domain = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[PAC] = pacman_domain

    ghost_domain = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[GHOST0] = ghost_domain
    variableDomainsDict[GHOST1] = ghost_domain
    
    observation_domain = list(range(MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim in eliminationOrder:
            factors, newFactor = joinFactorsByVariable(factors, elim)
            if len(newFactor.unconditionedVariables()) > 1:
                tmp_factor = eliminate(newFactor, elim)
                factors.append(tmp_factor)
            res_factor = joinFactors(factors)

        return normalize(res_factor)
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"

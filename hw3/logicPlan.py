...
def sentence2() -> Expr:
    ...
    cnd3 = ~(B & ~C) >> A
    cnd4 = ~D >> C
    return conjoin(cnd1, cnd2, cnd3, cnd4)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    dt_ptr =  a.__dict__
    dt_ptr['op'] = dt_ptr['op'].lower()
    # a.__dict__['op'] = a.__dict__['op'].lower()
    return {a: True}
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    result = premise & ~conclusion # p→q  p∧¬q  material implication
    
    return False if findModel(result) else True
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i, expr_i in enumerate(literals):
        for expr_j in literals[i+1:]:
            cj.append(disjoin(~expr_i, ~expr_j))    
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    wall_list = [PropSymbolExpr(pacman_str, wall_coordinate[0], wall_coordinate[1], time = t) for wall_coordinate in non_outer_wall_coords]
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        pacman_locations = exactlyOne([PropSymbolExpr(pacman_str, wall_coord[0], wall_coord[1], time = t) for wall_coord in non_wall_coords])
...
def foodLogicPlan(problem) -> List:
    ...
    for food_coords in food:
        KB.append(PropSymbolExpr(food_str, food_coords[0], food_coords[1], time = 0))
    ...
    for t in range(50):
        ...
        for food_coord in food:
            ...
            next_food = PropSymbolExpr(food_str, food_coord[0], food_coord[1], time = t+1)
